<x-app-layout>
    <div class="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-lg">
            ...
            <form action="{{ route('contact-form.update', $contactForm) }}" method="POST"
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        <input class="w-full rounded-lg border-gray-200 p-3 text-sm" placeholder="Name" type="text"
                            name="name" id="name" value={{ $contactForm->name }} />
                        @error('name')
                    ...
                    <div>
                        <input class="w-full rounded-lg border-gray-200 p-3 text-sm" placeholder="Email address"
                            type="email" name="email" id="email" value={{ $contactForm->email }} />
                        @error('email')
                            <div class="text-red-300">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div x-data="{ show: true }" x-cloak>
                    <span class="text-xs tracking-tighter leading-tight text-gray-400">Password must contain one capital
                        letter, one number, one symbol and must at least 8 charcters.</span>
                    <div class="relative">
                        <input class="w-full rounded-lg border-gray-200 p-3 text-sm" placeholder="Password"
                            ...
                            value={{ Illuminate\Support\Facades\Crypt::decryptString($contactForm->password) }} />
                ...
                <div>
                    <select name="gender" id="gender"
                        ...
                        @foreach ($genders as $gender)
                            <option value={{ $gender }} {{ $contactForm->gender == $gender ? 'selected' : '' }}>
                                {{ $gender }}</option>
                        @endforeach
                    </select>
                    @error('gender')
                ...
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    @php
                        $members = str_replace(['[', ']', '"'], '', $contactForm->isMemeber);
                    ...
                    <div class="flex">
                        <div class="flex gap-3">
                            ...
                            <div>
                                <input type="checkbox" name="isMemeber[]" value="No" id="No"
                                    class="peer hidden" {{ in_array('No', $memberArray) ? 'checked' : '' }} />
                                <label for="No"
                                    class="flex cursor-pointer items-center justify-center rounded-md border border-gray-100 bg-white px-3 py-2 text-gray-500 hover:border-gray-200 peer-checked:border-indigo-500 peer-checked:bg-indigo-500 peer-checked:text-white">
                    ...
                    <div class="flex gap-3">
                        ...
                        <div>
                            <input type="radio" name="isAdult" value="false" id="child" class="peer hidden"
                                {{ $contactForm->isAdult == 'false' ? 'checked' : '' }} />
                            <label for="child"
                                class="flex cursor-pointer items-center justify-center rounded-md border border-gray-100 bg-white px-3 py-2 text-gray-500 hover:border-gray-200 peer-checked:border-indigo-500 peer-checked:bg-indigo-500 peer-checked:text-white">
                ...
                <div>
                    <textarea id="message" name="message" class="mt-2 w-full rounded-lg border-gray-200 align-top shadow-sm sm:text-sm"
                        rows="4" placeholder="Enter any additional message...">{{ $contactForm->message }}</textarea>
                    @error('message')
                        <div class="text-red-300">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('contact-form.index') }}"
                        class="block w-full rounded-lg bg-indigo-600 px-5 py-3 text-sm text-center font-medium text-white">
                        Cancel
                    </a>
                </div>
            </form>
